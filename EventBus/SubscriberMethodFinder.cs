...
namespace Snork.EventBus
{
    internal class SubscriberMethodFinder
    {
        ...
        {
            var subscriberMethods = MethodCache.ContainsKey(subscriberClass) ? MethodCache[subscriberClass] : null;
            ...
            if (!subscriberMethods.Any())
                throw new EventBusException(
                    $"Subscriber {subscriberClass} and its base classes have no public methods with the @Subscribe annotation");

            MethodCache[subscriberClass] = subscriberMethods;
            return subscriberMethods;
        }

        private List<SubscriberMethod> FindUsingInfo(Type subscriberClass)
        {
        ...
        private ISubscriberInfo? GetSubscriberInfo(FindState findState)
        {
            if (findState.SubscriberInfo != null && findState.SubscriberInfo.GetSuperSubscriberInfo() != null)
            {
                ...
                if (findState.Type == superclassInfo.SubscriberClass) return superclassInfo;
            }
        ...
        internal class FindState
        {
            ...
            readonly Dictionary<string, Type> _subscriberClassByMethodKey = new Dictionary<string, Type>();
            public bool SkipSuperClasses { get; set; }

            private Type? subscriberClass;
            ...
            {
                subscriberClass = Type = subscriberType;
            ...
            private bool CheckAddWithMethodSignature(MethodInfo method, Type messageType)
            {
                ...
                var methodClassOld = _subscriberClassByMethodKey.Put(methodKey, methodClass);
                if (methodClassOld == null || methodClassOld.IsAssignableFrom(methodClass))
                    // Only Add if not already found in a sub class
                    return true;

                // Revert the put, old class is further down the class hierarchy
                _subscriberClassByMethodKey[methodKey] = methodClassOld;
                return false;
            }
